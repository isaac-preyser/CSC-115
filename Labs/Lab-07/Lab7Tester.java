...
public class Lab7Tester {
	...
	public static void testBasicStack() {
		...
		//throws an exception when data == -1 (or is empty). therefore test should pass if exception is thrown. 
		s.makeEmpty();
		try {
			s.pop();
	...
	public static String reverseString(String str) {
		...
		while (!stk.isEmpty()) {
			try {
				...
				//this probably shouldnt get run, as the condition for the loop is that the stack is not empty. 
				result = "error";
				return result;
			}
		}
			...
			if (c == '(') {
				...
				if (stk.isEmpty()) {
					...
					try {
						...
						//this probably shouldnt get run, as the condition for this is that the stack is not empty. 
						return false;
					}
				}
			}
	...
	public static void testQueue() {
		...
		//throws an exception on a empty queue. therefore test should pass if exception is thrown.
