...
public class IntegerLinkedList {
	...
	private void doubleValuesRec(IntegerNode n) {
		if (n == null) {
			return;
		} else {
			int doubleVal = n.getData() * 2;
			n.setData(doubleVal);
			doubleValuesRec(n.next);
			/*return*/ 
			//do I need a return statement here? (for valid/safe java syntax?)
		}
	...
	//odd data value is a hilarious way to say odd number 
	...
	private void doubleOddValuesRec(IntegerNode n) {
		if (n == null){
			return;
		} else {
			if (n.getData() % 2 != 0) {
...
/* 
	  ...
	  - a very confused csc115 student		
*/
	...
	private int sum(IntegerNode n) {
		if (n==null) {
			...
			int first = n.getData();
			int sumRest = sum(n.next); //this is kinda neat
	...
	private boolean allNegativeRec(IntegerNode n) {
		if (n == null) {
			return true;
		} else {
			if (n.getData() >= 0) {
	...
	private boolean isSortedAscendingRec(IntegerNode n) {
		if (n == null) {
			return true;
		} else {
			if (n.next != null) {
